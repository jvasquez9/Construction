...
public class andL_Ex extends AndParent {
	...
	public String and(String leftHandSide, String rightHandSide) {
		...
		if (specialCharPos == 0) {
			...
			String ActualPart = leftHandSide.substring(STARTINDEX,
					specialCharPos + ONECHARFLAG);
			String lastPart = leftHandSide.substring(specialCharPos
					+ ONECHARFLAG);

			btr = new StringBuilder(ActualPart);

			// Appended &P when identified ')' which means there is a scenario
			// like '()' case
			btr = handleCloseOpen(btr);
			for (int i = 0; i < btr.length(); i++) {
				if (btr.toString().contains("X")
						&& !btr.toString().contains("U")) {
					// This is only to handle &X scenario, only &X exist in the
					// formula
					btr = handleandX(btr);
					break;
				}
				if (!btr.toString().contains("X")
					...
					// This is only to handle &U scenario, only &U exist in the
					// formula
	...
	/*
	 * Handle &X scenario, when &X found in a string split string and then
	 * appended &Q at each substrings if they ends with '&'
	 */
	...
	/*
	 * Handle &U scenario, when &U found in a string split string and then
	 * appended &Q at each substrings if they doesn't ends with 'Q' or 'X'
	 */
	...
	public int getFirstSpecialCharPositionFromEnd(String leftHandString) {
		...
		for (; indexPos >= 0; indexPos--) {
			if (leftHandString.charAt(indexPos) == Symbols.NEXT
					.charAt(STARTINDEX)
					|| leftHandString.charAt(indexPos) == Symbols.UNTIL
							.charAt(STARTINDEX))
				return indexPos;
		}
		return 0;
